...
{
    private readonly NetworkPathResolver _resolver = new();
    ...
    [TestMethod]
    public void LoopbackToAnotherProcessListenerIsMarkedLocalProxy()
    {
        NetworkTrafficEvent item = new(DateTimeOffset.UtcNow, 10, "TCP", TrafficDirection.Upload, 10, "127.0.0.1", 5000, "127.0.0.1", 7890);
        NetworkConnectionSnapshot listener = new(20, "TCP", "127.0.0.1", 7890, "0.0.0.0", 0, "Listen", true);
        NetworkPathResolution result = _resolver.Resolve(item, [listener], new Dictionary<int, string> { [20] = "Clash Verge" });
        Assert.AreEqual(NetworkPathKind.LocalProxy, result.Kind);
        Assert.AreEqual("Clash Verge", result.ProxyProcessName);
    }
